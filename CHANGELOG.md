...
### 수정
- **대용량 파일 비교 시 UI 먹통 문제 수정** — 독립 원인 3종: (1) A↔B 스크롤 동기 무한 재진입
  가드 추가, (2) 긴 셀 char-diff 길이 상한(paint 정지 방지), (3) 대량 행 숨김을 O(R²)→O(R)로 개선.
- 키 열/행(틀 고정) 헤더 우클릭에도 A/B 병합 준비·병합 취소 메뉴 노출.

## [188] - 2026-07-15

### 추가 / 수정
- **CSV/TSV 네이티브 비교**(비교 전용) — .csv/.tsv를 엑셀처럼 A/B 비교. 인코딩(utf-8/cp949)·구분자 자동.
- JSON 문자열 값의 **개행 코드(`\n`) 보존** — 실제 줄바꿈으로 자동 처리하지 않고 리터럴 코드 그대로 표시.
- 병합 저장 시 **`.bak` 백업 파일 생성 중단**(셀·폴더 병합 모두).

### 내부
- 조용히 삼킨 오류에 대한 진단 로깅 추가(`%APPDATA%/ExcelMerge/excelmerge.log`).

## [187] - 2026-07-15

### 수정
- 다중 셀 선택으로 병합 준비 시, 틀 고정으로 숨겨진 **키 열/행 셀이 누락**되던 문제 수정 —
  신규 행 복사 시 키 값(UniqueID 등)이 빠지지 않도록 선택 행/열에 키 열·행을 보충.
- 스크롤을 맨 아래/오른쪽 끝까지 내렸을 때 **틀 고정된 키 열/행이 본체와 한 칸 어긋나 보이던**
  정렬 문제 수정.
